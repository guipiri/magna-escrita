...
import { createPrice } from '../../services/prices-service';
import { CreatePriceTierRequest } from '@repo/shared';
...
export function CreatePriceDialog({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data) => {
      enqueueSnackbar(
        `Preço "${data.name || data.id}" criado com sucesso!`,
        { variant: 'success' }
      );
      // Reset state
      setName('');
      setTiers([{ minQuantity: 1, unitPrice: 39.9 }]);
      setSelectedClassIds([]);
      setClassSearch('');
      setValidationError(null);
          ...
          <div className='p-3 bg-destructive/10 border border-destructive/20 text-destructive rounded-xl text-sm flex items-start gap-2'>
            ...
            <div>
              {validationError ||
                ...
                'Ocorreu um erro ao criar a tabela de preço.'}
            </div>
          </div>
        )}
          ...
          <div className='space-y-3'>
            ...
            <div className='relative'>
              ...
              <Input
                ...
                className='pl-9 h-9'
                disabled={classesLoading || createMutation.isPending}
              />
            </div>
